            ...
            <form onSubmit={handleSubmit}>
                ...
                <Card>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                        <h2 style={{ fontSize: '1.25rem' }}>Art├¡culos / Herramientas</h2>
                    ...
                    {formData.articles.map((art, index) => (
                        <div key={index} style={{ borderBottom: '1px solid var(--border)', paddingBottom: '20px', marginBottom: '20px' }}>
                            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 40px', gap: '12px', alignItems: 'end' }}>
                                <Input label="Descripci├│n" name="descripcion" value={art.descripcion} onChange={(e) => handleArticleChange(index, e)} required />
                    ...
                    {formData.articles.length === 0 && <p style={{ color: 'var(--text-muted)', textAlign: 'center', fontSize: '0.875rem' }}>No hay art├¡culos registrados.</p>}
                </Card>

                <Card>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                        <h2 style={{ fontSize: '1.25rem' }}>Documentaci├│n</h2>
                    ...
                    {formData.documents.map((doc, index) => (
                        <div key={index} style={{ borderBottom: '1px solid var(--border)', paddingBottom: '20px', marginBottom: '20px' }}>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr 40px', gap: '12px', alignItems: 'end' }}>
                                ...
                                <Input label="Observaci├│n" name="descripcion" value={doc.descripcion} onChange={(e) => handleDocumentChange(index, e)} />
                ...
                <Card>
                    <h2 style={{ fontSize: '1.25rem', marginBottom: '20px' }}>Autorizaci├│n</h2>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '20px' }}>
                        <Select
                            label="Motivo"
                            name="motivo"
                            ...
                            onChange={handleMovChange}
                            options={['Motivos personales', 'Requerimiento laboral', 'Accidente o razones m├®dicas', 'Otros']}
                            required
                        />
                        <Select
